...
(defn new-local-storage [config]
  #?(:cljs (map->LocalStorage {:content-key (get config :content-key)})
     :clj  (map->LocalStorage {:directory   (get config :directory)
                               :content-key (get config :content-key)})))
